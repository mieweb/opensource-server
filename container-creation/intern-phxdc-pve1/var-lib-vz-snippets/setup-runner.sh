...
if [ ! -z "$CONTAINER_OWNERSHIP" ]; then
    outputError 1 "You already own a container with name \"$CONTAINER_NAME\". What happened to the runner \"$CONTAINER_NAME\" in your repository? Either remove the setup-runner step from your workflow or delete the pre-existing container and try again."
fi
...
pveum aclmod /vms/$NEXT_ID --user "$PROXMOX_USERNAME@pve" --role VMUser2  > /dev/null 2>&1
