...
removeTempEnvVars() {
    TEMP_ENV_FOLDER="/var/lib/vz/snippets/container-env-vars"
    while read -r line; do
        if [[ "$line" == /var/lib/vz/snippets/container-env-vars/env_* ]]; then
...
removeTempServices() {
    TEMP_SERVICES_FOLDER="/var/lib/vz/snippets/container-services"
    while read -r line; do
        if [[ "$line" == /var/lib/vz/snippets/container-services/services_* ]]; then
            rm -f "$line"
...
removeTempPublicKeys() {
    TEMP_PUB_FOLDER="/var/lib/vz/snippets/container-public-keys"
    while read -r line; do
        if [[ "$line" == /var/lib/vz/snippets/container-public-keys/key_* ]]; then
            rm -f "$line"
...
removeTempProtocols() {
    TEMP_PROTOCOL_FOLDER="/var/lib/vz/snippets/container-port-maps"
    while read -r line; do
        if [[ "$line" == /var/lib/vz/snippets/container-port-maps/protocol_list* ]]; then
            rm -f "$line"
