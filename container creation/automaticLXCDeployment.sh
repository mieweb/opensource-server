...
if [ -f "/var/lib/vz/snippets/container-services/$SERVICES_BASE_FILE" ]; then
	while read line; do
		pct exec $NEXT_ID -- bash -c "$line" > /dev/null
...
pct set $NEXT_ID --memory 4096 --swap 0 --cores 4 > /dev/null #temporarily bump up container resources for computation hungry processes (e.g. meteor)\
	...
	if [ "${RUNTIME_LANGUAGE^^}" == "NODEJS" ]; then
		...
		if [ "$BUILD_COMMAND" == "" ]; then
			pct exec $NEXT_ID -- bash -c "export PATH=\$PATH:/usr/local/bin && cd /root/$REPO_BASE_NAME/$PROJECT_ROOT && pm2 start bash -- -c '$START_COMMAND'" > /dev/null
		else
			pct enter $NEXT_ID <<EOF > /dev/null
cd /root/$REPO_BASE_NAME/$PROJECT_ROOT && \
...
EOF
		...
		if [ "$BUILD_COMMAND" == "" ]; then
			pct exec $NEXT_ID -- "cd /root/$REPO_BASE_NAME/$PROJECT_ROOT && $START_COMMAND" > /dev/null
		else
