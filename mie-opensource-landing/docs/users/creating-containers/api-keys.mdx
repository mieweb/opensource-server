...
|---------|----------|-----------------|
| Authentication method | `Authorization: Bearer` header | Browser cookies |
| Best for | Automation, scripts, integrations | Interactive web browsing |
| Expiration | Never (until revoked) | 24 hours of inactivity |
| Requires password | No | Yes (at login) |
| Multiple concurrent access | Yes (one key per application) | Limited by session |
| Easy to revoke | Yes (per key) | Requires re-authentication |
