...
1. **API Keys** (Recommended for automation) - See [API Keys](./api-keys.mdx) for detailed documentation
2. **Session Cookies** (For interactive sessions) - Documented below

:::tip Recommendation
For scripts, automation, and CI/CD pipelines, use API keys instead of session cookies. API keys don't expire and can be easily revoked without changing your password.
:::

## 1. Authenticate and Establish Session

### Option A: Using an API Key (Recommended)

If you have an API key, include it in the `Authorization` header for all requests:
...
### Option B: Using Session Cookies

Alternatively, log in to establish a session. The API uses cookie-based authentication:
